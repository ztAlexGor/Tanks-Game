...
class Map():
    ...
    def checkObstacle(self, x, y, width, height): # 0 - empty, 1 - unbreak. wall, 2 - breakeable wall, 3 - water
        ...
        if x < 0 or x >= 1500 or y < 0 or y >= 900:
            return 1
        if x < 0 or x >= 1500 or y + height - 1 < 0 or y + height - 1 >= 900:
            return 1
        if x + width - 1 < 0 or x + width - 1 >= 1500 or y < 0 or y >= 900:
            return 1
        if x + width - 1 < 0 or x + width - 1 >= 1500 or y + height - 1 < 0 or y + height - 1 >= 900:
            return 1
    ...
    def getRandomEmptyPoint(self):
        while True:
            x, y = random.randint(0, 14), random.randint(0, 8)
            if self.terrain[y][x] in [0,1,8]:
                return (x, y)